/* @hash e1d50b565235923e413c68157739cd76 */
// tslint:disable
/* eslint-disable */
import {
  addLocalKeysSync,
...
> => {
  ...
  if (process.env.NODE_ENV !== 'production' || process.env.NEO_ONE_DEV === 'true') {
    providers.push({ network: 'local', rpcURL: `http://${host}:9040/rpc` });
  }
...
export const createDeveloperClients = (host = 'localhost'): DeveloperClients => ({
  local: new DeveloperClient(new NEOONEDataProvider({ network: 'local', rpcURL: `http://${host}:9040/rpc` })),
});
